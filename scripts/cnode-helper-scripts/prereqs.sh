...
if [ "$WANT_BUILD_DEPS" = 'Y' ]; then
  ...
  if [[ "${OS_ID}" =~ ebian ]] || [[ "${DISTRO}" =~ ebian ]]; then
    ...
    ! grep -q ^epel <<< "$(yum repolist)" && sudo yum -y install https://dl.fedoraproject.org/pub/epel/epel-release-latest-"$(grep ^VERSION_ID /etc/os-release | cut -d\" -f2 | cut -d. -f1)".noarch.rpm > /dev/null
